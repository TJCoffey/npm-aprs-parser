...
module.exports = {
    parsePositionAndCommentString: function (stringPartWithPosAndComment) {
        ...
        if (comment && comment.length > 0) {
            ...
            if (parsedFromComment.dao && position) {
                position.setDatum(parsedFromComment.dao.datum);
                if (position.latitude >= 0)
                    position.latitude += parsedFromComment.dao.latAdjust;
                else
                    position.latitude -= parsedFromComment.dao.latAdjust;
                if (position.longitude >= 0)
                    position.longitude += parsedFromComment.dao.lonAdjust;
                else
                    position.longitude -= parsedFromComment.dao.lonAdjust;
            }
