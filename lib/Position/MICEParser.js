...
const DAOUtils = require('./DAOParserUtils.js');
...
MICEParser.prototype.tryParse = function (bodyContent, aprsMessageHeader) {
    ...
    if (comment) {
        ...
        if (comment) {
            const daoData = DAOUtils.parseDAO(comment);
            if (daoData) {
                console.log("Comment: " + daoData.comment);
                comment = daoData.comment;
                position.setDatum(daoData.datum);
                if (position.latitude >= 0)
                    position.latitude += daoData.latAdjust
                else
                    position.latitude -= daoData.latAdjust
                if (position.longitude >= 0)
                    position.longitude += daoData.lonAdjust
                else
                    position.longitude -= daoData.lonAdjust
            }
        }
